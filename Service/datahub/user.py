...
from Api.dh_api.register_api import *
...
class User:
    ...
    def check_user_list(self):
        ...
        a_id = res.get('data').get('records')[0].get('accountId')
        account_email=res.get('data').get('records')[0].get('email')
        user_status = res.get('data').get('records')[0].get('status')

        # role_list = res.get('data').get('records')[0].get('roleNames')
        YamlUtil().write_yaml({'account_email':account_email})
        assert user_status == 1
        return {'a_id': a_id,'account_email':account_email}



if __name__ == '__main__':
    data = User().check_user_list()
    print(data)
